...
show-access-av-form()
{
    ...
    blankItemContent[0]=""
    blankItemContent[1]="internal external auxiliary vpnipsec"
    ...
    listContent=${ACCESS_AV_LIST}
    test -n "${listContent}" || state=disabled
    
    show-title "Allowed Antivirus Client IPs" "${state}" "access"
    show-list-form ${MAX_ACL_AV_NB} "${width}" "${page_ref}"
}
