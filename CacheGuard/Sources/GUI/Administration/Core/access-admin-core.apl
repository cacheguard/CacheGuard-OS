...
show-access-admin-form()
{
    ...
    listContent=${ACCESS_ADMIN_LIST}
    listContentStep=4
    listContentVisibility[3]=off

    test -n "${listContent}" || state=disabled

    show-title "Allowed Remote Administrator IPs" "${state}" "access admin"
    show-list-form ${MAX_ACL_ADMIN_NB} "${width}" "${page_ref}"
}
